...
from mlp import ProjectionMLP, OrMLP, AndMLP, NotMLP
from mlpMixer import ProjectionMixer, OrMixer, AndMixer
    ...
    def forward(self, embeddings):
        if self.use_att:  # use attention with weighted t-norm
            ...
            if self.tnorm == 'mins':  # minimum / Godel t-norm
                ...
                if self.bounded:
                    embedding = order_bounds(embedding) #(lower,upper)
...
class LogicProjection(nn.Module):
    def __init__(self, entity_dim, relation_dim, hidden_dim, num_layers, bounded, with_sigmoid=False):
        ...
        self.num_layers = num_layers
        self.bounded = bounded
        self.layer1 = nn.Linear(self.entity_dim + self.relation_dim, self.hidden_dim)  # 1st layer;F1
        self.layer0 = nn.Linear(self.hidden_dim, self.entity_dim)  # final layer;F3
        self.with_sigmoid = with_sigmoid
        for nl in range(2, num_layers + 1):
            setattr(self, "layer{}".format(nl), nn.Linear(self.hidden_dim, self.hidden_dim)) # F2
        for nl in range(num_layers + 1):
            nn.init.xavier_uniform_(getattr(self, "layer{}".format(nl)).weight)
...
class KGReasoning(nn.Module):
    def __init__(self, nentity, nrelation, ntype, hidden_dim, entity2type, relation2type, gamma,
                 ...
                 beta_mode=None, center_reg=None, logic_mode=None, mlp_mode=None,
                 model_mode='baseline', drop=0., neighbor_ent_type_samples=32, neighbor_rel_type_samples=64,
                 negation_mode='baseline', negation_together_enhance='together', enhance_mode='domain_range',
                 range_enhance_mode=None):
        """
            :param nentity: 实体数
            :param nrelation: 关系数
            :param ntype: 类型数
            :param hidden_dim: 编码维度，embedding dimension
            :param entity2type: 实体类型 [[],[],[]...]
            :param relation2type: 关系类型 [[],[],[]...]
            :param gamma: default: 0.375
            :param geo: 选择的base模型：['vec', 'box', 'beta', 'cone', 'logic']
            :param test_batch_size: valid/test batch size
            :param box_mode: default: (none,0.02), (offset activation,center_reg) for Query2box, center_reg balances the in_box dist and out_box dist
            :param use_cuda: 是否使用cuda
            :param query_name_dict: {query_structure:query_type}, {tuple:str,tuple:str,tuple:str...}
            :param beta_mode: default: (1600,2), (hidden_dim,num_layer) for BetaE relational projection
            :param center_reg: center_reg for ConE, center_reg balances the in_cone dist and out_cone dist
            :param logic_mode: default: (luk,1,1,0,1600,2), (tnorm,bounded,use_att,use_gtrans,hidden_dim,num_layer)
            :param mlp_mode: mlp的Linear层数
            :param model_mode: the type of model: ['baseline', 'temp']
            :param drop: dropout rate
            :param neighbor_ent_type_samples: number of sampled entity type neighbors
            :param neighbor_rel_type_samples: number of sampled relation type neighbors
            :param negation_mode: 是否启用negation增强
            :param negation_together_enhance: negation增强方式: ['together', 'separate']
            :param enhance_mode: query增强方式: ['domain_only', 'domain_range', 'domain_range_plus_origin']
            :param range_enhance_mode: 值域增强方式: ['attention_weight', 'None']
        """
        ...
        if model_mode == 'temp':
            self.range_enhance_mode = range_enhance_mode
            ...
            self.neighbor_rel_type_samples = neighbor_rel_type_samples

            if self.geo == 'vec' or self.geo == 'box' or self.geo == 'cone' or self.geo == 'mlp' or self.geo == 'mlpMixer': # GQE,Q2B,mlp,mlpMixer
                ...
                if enhance_mode == 'domain_range' or enhance_mode == 'domain_range_plus_origin':
                    if self.range_enhance_mode == 'attention_weight':
                        self.ent_relation_type_agg = EntityTypeAggregator(input_dim=hidden_dim, output_dim=hidden_dim,
                                                                          self_included=True,
                                                                          neighbor_ent_type_samples=1)
                    else:
                        self.ent_relation_type_agg = EntityTypeAggregator(input_dim=hidden_dim, output_dim=hidden_dim,
                                                                          self_included=True,
                                                                          neighbor_ent_type_samples=self.neighbor_rel_type_samples)
            elif self.geo == 'beta' or self.geo == 'logic':
                self.negation_together_enhance = negation_together_enhance
                ...
                if enhance_mode == 'domain_range' or enhance_mode == 'domain_range_plus_origin':
                    if negation_together_enhance == 'together':
                        if self.range_enhance_mode == 'attention_weight':
                            self.ent_relation_type_agg = EntityTypeAggregator(input_dim=hidden_dim * 2, output_dim=hidden_dim * 2,
                                                                              self_included=True,
                                                                              neighbor_ent_type_samples=1)
                        else:
                            self.ent_relation_type_agg = EntityTypeAggregator(input_dim=hidden_dim * 2,
                                                                              output_dim=hidden_dim * 2,
                                                                              self_included=True,
                                                                              neighbor_ent_type_samples=self.neighbor_rel_type_samples)

                    elif negation_together_enhance == 'separate':
                        if self.range_enhance_mode == 'attention_weight':
                            self.ent_relation_type_agg = EntityTypeAggregator(input_dim=hidden_dim, output_dim=hidden_dim,
                                                                              self_included=True,
                                                                              neighbor_ent_type_samples=1)
        ...
        self.together_enhance = negation_together_enhance
        ...
        if self.geo in ['box', 'vec', 'beta', 'cone', 'mlp', 'mlpMixer']:
            nn.init.uniform_(
        ...
        if self.geo == 'box':
            ...
            if self.range_enhance_mode == 'attention_weight':
                self.range_type_center_net = CenterIntersection(self.entity_dim)
            ...
            if self.range_enhance_mode == 'attention_weight':
                self.range_type_center_net = CenterIntersection(self.type_dim)
            ...
            self.notNN = NotMLP(mlp_mode, self.entity_dim)
            self.projectionNN = ProjectionMLP(mlp_mode, self.entity_dim)
            self.andNN = AndMLP(mlp_mode, self.entity_dim)
            if self.model_mode == 'temp':
                self.type_embedding = nn.Parameter(torch.zeros(self.ntype, self.type_dim))
                nn.init.uniform_(
                    tensor=self.type_embedding,
                ...
                self.center_net = CenterIntersection(self.type_dim)

                if self.range_enhance_mode == 'attention_weight':
                    self.range_type_center_net = CenterIntersection(self.type_dim)
        elif self.geo == 'mlpMixer':
            self.projectionNN = ProjectionMixer(mlp_mode, self.entity_dim)
            self.andNN = AndMixer(mlp_mode, self.entity_dim)
    ...
    def forward(self, positive_sample, negative_sample, subsampling_weight, batch_queries_dict, batch_idxs_dict):
        if self.geo == 'box':
            ...
            return self.forward_mlp(positive_sample, negative_sample, subsampling_weight, batch_queries_dict, batch_idxs_dict)

    def embed_query_vec(self, queries, query_structure, idx):
        '''
        Iterative embed a batch of queries with same structure using GQE
        queries: a flattened batch of queries: list[query,query,...]
        query_structure: tuple, eg: ('e',('r',))
        idx:
        '''
    ...
    def embed_query_mlp(self, queries, query_structure, idx):
        '''
        Iterative embed a batch of queries with same structure using GQE
        queries: a flattened batch of queries
        '''
        ...
        if all_relation_flag:
            if query_structure[0] == 'e':
                if self.model_mode == 'baseline':
                    ...
                    embedding = torch.index_select(self.entity_embedding, dim=0, index=queries[:, idx])
                    ent_type_id = torch.index_select(self.entity2type, dim=0, index=queries[:, idx])
                    entity_neighbor_type_embedding = torch.index_select(self.type_embedding, dim=0,
                                                                        index=ent_type_id.view(-1)).view(
                                                                            ent_type_id.shape[0], ent_type_id.shape[1], -1)
                    embedding = self.ent_neighbor_type_agg(embedding, entity_neighbor_type_embedding)
                idx += 1
            else:
                embedding, idx = self.embed_query_mlp(queries, query_structure[0], idx)
            for i in range(len(query_structure[-1])):
                if query_structure[-1][i] == 'n':
                    ...
                    if self.geo == 'mlpMixer':
                        ...
                        if self.negation_mode == 'enhance':
                            embedding = self.domain_enhance_(embedding, queries, idx - 1)
                else:
                    if self.model_mode == 'baseline':
                        r_embedding = torch.index_select(self.relation_embedding, dim=0, index=queries[:, idx])
                        ...
                        r_embedding = torch.index_select(self.relation_embedding, dim=0, index=queries[:, idx])
                        rel_type_id = torch.index_select(self.relation2type, dim=0, index=queries[:, idx])
                        relation_neighbor_type_embedding = torch.index_select(self.type_embedding, dim=0,
                                                                              index=rel_type_id.view(-1)).view(
                                                                                rel_type_id.shape[0], rel_type_id.shape[1], -1)
                        relation_neighbor_type_embedding = torch.transpose(relation_neighbor_type_embedding, 0, 1)
                        rel_type_cent = self.center_net(relation_neighbor_type_embedding)
                        embedding = embedding.unsqueeze(1)
                        r_embedding = r_embedding.unsqueeze(1)
            ...
            embedding_list = []
            for i in range(len(query_structure)):
                embedding, idx = self.embed_query_mlp(queries, query_structure[i], idx)
                embedding_list.append(embedding)
    ...
    def forward_logic(self, positive_sample, negative_sample, subsampling_weight, batch_queries_dict, batch_idxs_dict):
        ...
        if len(all_embeddings) > 0:
            ...
            if positive_sample is None and self.bounded:
    ...
    def forward_mlp(self, positive_sample, negative_sample, subsampling_weight, batch_queries_dict, batch_idxs_dict):
        ...
        for query_structure in batch_queries_dict:
            if 'u' in self.query_name_dict[query_structure]:
                center_embedding, _ = self.embed_query_mlp(
                ...
                center_embedding, _ = self.embed_query_mlp(batch_queries_dict[query_structure], query_structure, 0)
                all_center_embeddings.append(center_embedding)
                all_idxs.extend(batch_idxs_dict[query_structure])
    ...
    def train_step(model, optimizer, train_iterator, args, step):
        ...
        # positive_logit, negative_logit, subsampling_weight, all_idxs+all_union_idxs, None
        ...
        # compute positive_sample_loss & negative_sample_loss
        # loss function
        ...
        with torch.no_grad(): # disable gradient calculation
            ...
            for negative_sample, queries, queries_unflatten, query_structures in tqdm(test_dataloader, disable=not args.print_on_screen):
                ...
                for i, query in enumerate(queries):
                    batch_queries_dict[query_structures[i]].append(query)
                    batch_idxs_dict[query_structures[i]].append(i)
                # 将数据转为torch.LongTensor()类型 并判断是否转移到cuda
                for query_structure in batch_queries_dict:
                    if args.cuda:
                        batch_queries_dict[query_structure] = torch.LongTensor(batch_queries_dict[query_structure]).cuda()
                    else:
