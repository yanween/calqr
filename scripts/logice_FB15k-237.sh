#!/bin/bash
#SBATCH -J logice_FB15k-237
#SBATCH --output ../logs/FB15k-237-operation2/logice_nram_domain_only_pretrain_retrain.out
#SBATCH -p batch
#SBATCH -N 1
...
export SAVE_PATH=../logs/FB15k-237-operation2/logice_nram_domain_only_pretrain_retrain
export LOG_PATH=../logs/FB15k-237-operation2/logice_nram_domain_only_pretrain_retrain.out
export MODEL=temp
...
python -u ../main.py --cuda --do_train --do_valid --do_test --do_pretrain --do_prevalid --do_pretest \
  ...
  --cpu_num 6 --geo logic -logicm "(luk,0,1,0,1600,2)" --test_batch_size 32 --tasks "1p.2p.3p.2i.3i.ip.pi.2u.up" --print_on_screen \
